...
Released on March 31, 2023.
 ...
 -  `BlockChain<T>.Create()` is now get `IBlockChainState` and
    `BlockExecutor` as parameters.  [[#3029]]
...
### Backward-incompatible API changes
 ...
 -  Removed type parameter `T` for `IAction` from `BlockExecutor` and
 ...
 -  `BlockExecutor()` constructor now explicitly requires a new
...
### Backward-incompatible API changes
 ...
 -  `BlockChain<T>()` no longer accepts an `Libplanet.Data.Store` where
    `Libplanet.Data.Store.GetCanonicalChainId()` is `null`.  For on-the-fly `BlockChain<T>`
 ...
 -  `BlockChain<T>()` now throws an `ArgumentException` if provided `Libplanet.Data.Store`
    does not have its canonical chain id set or provided chain id is not found.
    [[#1486], [#2585], [#2889]]
 -  Added `Libplanet.Data.Store.GetChainBlockCommit()` and `Libplanet.Data.Store.PutChainBlockCommit()`
    interface methods.  [[#2878], [#2894]]

### Added APIs

 -  Added `BlockChain<T>.Create()` factory method for creating a `BlockChain<T>`
    with an empty `Libplanet.Data.Store`.  [[#2863]]
...
### Deprecated APIs
 ...
 -  Removed `BlockPolicy<T>.GetNextBlockDifficulty()` interface method
    and all its implementations.  [[#2872]]
 -  Removed `BlockPolicy.TotalDifficulty` interface property and its
    implementations.  [[#2872]]
 -  Removed all total difficulty related parameters.  [[#2872]]
 -  Removed `BlockPolicy.Difficulty` interface property and its
    implementations.  [[#2872]]
 ...
 -  Removed `BlockPolicy.GetMinBlockProtocolVersion()` interface method.
    [[#2872]]

### Backward-incompatible API changes

 -  Added `LastCommit` property to `IBlockHeader`.  [[#2872]]
 -  Bumped `BlockHeader.CurrentProtocolVersion` to 4.  [[#2872]]
 -  Changed `IPreEvaluationBlockHeader.PreEvaluationHash` type from
    `ImmutableArray<byte>` to `HashDigest<SHA256>`.  [[#2872]]
 -  Added `Libplanet.Data.Store.GetBlockCommit(BlockHash)` method.  [[#2872]]
 -  Added `Libplanet.Data.Store.PutBlockCommit(BlockCommit)` method.  [[#2872]]
 -  Added `Libplanet.Data.Store.DeleteBlockCommit(BlockHash)` method.  [[#2872]]
 -  Added `Libplanet.Data.Store.GetBlockCommitHashes()` method.  [[#2872]]
 -  `BlockHeader.MakeCandidateData()` now uses a SHA256 hash of `LastCommit`
    ...
    computation if the `BlockHeader.LastCommit` is not null.  [[#2872]]
...
### Added APIs
 ...
 -  (Libplanet.Net) Added `InvalidOperationException` class.  [[#2872]]
 ...
 -  (Libplanet.Explorer) Added `VoteTypeType` class.  [[#2872]]
...
### Backward-incompatible API changes
 ...
 -  Removed `InvalidMessageException` class.
    Instead, added `InvalidMessageContractException` class.  [[#2772], [#2831]]
...
### Behavioral changes
 ...
 -  `BlockPolicy` is no longer enforced for genesis `Block<T>`s.  [[#2845]]
...
### Backward-incompatible API changes
 ...
 -  (Libplanet.Commands)  Dropped .NET Standard 2.0 and .NET Core 3.1
    target assemblies.  [[#2732]]
 -  (Libplanet.Commands)  Added .NET 6 target assembly.  [[#2732]]
...
### Added APIs
 ...
 -  Introduced *Libplanet.Types.Crypto.Secp256k1* package.  [[#2780]]
...
### Backward-incompatible API changes
 ...
 -  Replaced `IAction?`-typed `policyBlockAction` parameter with
    ...
    in `BlockExecutor` constructor.  [[#2646]]
 -  Removed `Libplanet.Data.Store.GetCanonicalGenesisBlock<T>()` interface method and all its
...
### Deprecated APIs
 ...
 -  Removed `BlockChain<T>.MineBlock(PrivateKey, DateTimeOffset, bool, long,
    ...
    it `internal`.  [[#2529]]
 -  Removed `Libplanet.Data.Store.SetBlockPerceivedTime()` and
    `Libplanet.Data.Store.GetBlockPerceivedTime()` methods.  [[#2575]]
...
### Added APIs
 ...
 -  Added new constructors of `BlockChain<T>` takes `IBlockChainStates<T>`
    and `BlockExecutor<T>` directly.  [[#2507]]
 ...
 -  (Libplanet.Explorer) Added a new GraphQL endpoint on `/graphql/explorer`.
    [[#2562]]
 -  Added `BlockPolicy.GetMinBlockProtocolVersion()` interface method.
    [[#2593]]
...
Released on November 1, 2022.
 ...
 -  (Libplanet.Data.RocksDB) `RocksDBStore` no more crashes with stack overflow
...
### Backward-incompatible API changes
 ...
 -  Overhauled constructors for `BlockHeader`, `BlockContent<T>`,
    ...
    and `Block<T>`.  [[#2321]]
     ...
     -  `BlockHeader` has constructors `BlockHeader(IBlockHeader)`
        and `BlockHeader(long, DateTimeOffset, PublicKey, long, BigInteger,
        BlockHash?, HashDigest<SHA256>)`.
     -  `BlockContent` has constructors `BlockContent<T>(IBlockHeader,
        IEnumerable<Transaction<T>> transactions)`,
        `BlockContent<T>(BlockHeader)` and `BlockContent<T>(BlockHeader,
     ...
     -  `PreEvaluationBlockHeader` has constructors
        ...
        `PreEvaluationBlockHeader(BlockHeader, (Nonce,
        ImmutableArray<byte>))`.
 ...
 -  `BlockContent<T>` no longer inherits `BlockHeader` and
    `PreEvaluationBlock<T>` no longer inherits `PreEvaluationBlockHeader`.
    [[#2321]]
 -  Both `BlockHeader` and `BlockContent<T>` are made immutable.  Their
    properties can no longer be assigned to.  [[#2321]]
 -  Copy extension methods for `BlockHeader` and `BlockContent<T>` removed.
    [[#2321]]
 -  (Libplanet.Commands) The return type of
...
### Added APIs
 ...
 -  (Libplanet.Net) `RoutingTable` now implements `IRoutingTable` interface.
    [[#2046], [#2229]]
 -  Added `BlockExecutor<T>.GenerateRandomSeed()` static method.
    [[#2131], [#2236]]
 -  Each `BlockHeader`, `PreEvaluationBlockHeader`, and `BlockHeader`
    can be accessed from any "larger" type object through properties.  [[#2321]]
     -  `BlockHeader` can be accessed through `BlockContent<T>.Metadata`
...
### Backward-incompatible API changes
 ...
 -  Changed `BlockPolicy.GetMaxBlockBytes()` to
    `BlockPolicy.GetMaxTransactionBytes()`.  Behaviourally, this is now used
...
### Backward-incompatible API changes
 ...
 -  Removed unused transaction related methods from `Libplanet.Data.Store` and its
 ...
 -  Reworked constructors of exception classes. Affected classes are:
     - (Libplanet.Net) `TimeoutException`
     ...
     `InvalidTxIdException`, `InvalidOperationException`,
     `InvalidOperationException`, `MissingActionTypeException`,
     `NoKeyException`, `SupplyOverflowException`,
     `TotalSupplyNotTrackableException`, `InvalidOperationException`,
...
### Deprecated APIs
 ...
 -  `IPreEvaluationBlockHeader.HashAlgorithm` property and its implementations
    removed.  [[#2206], [#2207]]
 -  `BlockPolicy.GetHashAlgorithm(long)` method and its implementations
    removed.  [[#2206], [#2207]]
...
### Backward-incompatible API changes
 ...
 -  Renamed parameters named `actions` of many methods to `customActions`.
    [[#2149], [#2151], [#2175]]
     ...
     -  Renamed `TxMetadata.ToBencodex(IEnumerable<IValue>,
        ...
        `customActions`.
 -  Added `BlockPolicy<T>.NativeTokens` property.  [[#2149], [#2150], [#2175]]
 ...
 -  Added parameter `Predicate<Currency> nativeTokenPredicate` to
    ...
    (parameter `TrieStateStore stateStore` remains at the last).
    [[#2149], [#2150], [#2175]]
 -  Added parameter `Predicate<Currency> nativeTokenPredicate` to
 ...
 -  Added parameter `Predicate<Currency> nativeTokenPredicate` to
    ...
    [[#2149], [#2150], [#2175]]
 -  Added parameter `Predicate<Currency> nativeTokenPredicate` to
    `BlockExecutor<T>()` constructor as its last parameter.
    [[#2149], [#2150], [#2175]]
 -  Removed `ChainIdNotFoundException` class.  [[#2047], [#2156]]
 -  Added `Libplanet.Data.Store.GetCanonicalGenesisBlock(HashAlgorithmGetter)` method.
    [[#2162], [#2171]]
 ...
 -  Parameter `HashAlgorithmType hashAlgorithm` removed from
    `BlockHeader<T>.DerivePreEvaluationHash()` method.  [[#2206], [#2207]]
 -  Parameter `HashAlgorithmType hashAlgorithm` removed from
    `BlockHeader<T>.MineNonce()` methods.  [[#2206], [#2207]]
 -  Return type for `BlockMarshaler
    .UnmarshalPreEvaluationBlockHeader(Dictionary)` changed from
    `(BlockHeader, Nonce, ImmutableArray<bytes>?)` to
 ...
 -  Parameter `HashAlgorithmGetter hashAlgorithmGetter` removed from
    `Libplanet.Data.Store<T>.GetCanonicalGenesisBlock<T>()` and
    `Libplanet.Data.Store<T>.GetBlock<T>()` interface methods and their implementations.
    [[#2206], [#2207]]
 -  Parameter `HashAlgorithmGetter hashAlgorithmGetter` removed from
...
### Added APIs
 ...
 -  Added `BlockExecutor<T>.GenesisHash` property.  [[#1972], [#2179]]
 -  Added `IAccountStateView` interface.  [[#2183]]
 -  `IAccountStateDelta` now inherits `IAccountStateView` interface.
    [[#2183]]
 -  Added `BlockHeader.HashAlgorithmType` static property.  [[#2206], [#2207]]
...
### Behavioral changes
 ...
 -  Nonexistent chain ids in `Libplanet.Data.Store` are now considered to be chain ids of
...
### CLI tools
 ...
 -  (Libplanet.Commands) Upgraded *Cocona.Lite* from 1.6.\* to
 ...
 -  (Libplanet.Commands) Removed `DerivationCommand` class.
    [[#2118]]
...
### Behavioral changes
 ...
 -  `MemoryStore` and `MemoryKeyValueStore`-backed `TrieStateStore` now can be
    ...
    `StoreLoaderAttribute.LoadStore()` method.  [[#1359], [#1953], [#1955]]
 -  (Libplanet.Data.RocksDB) `RocksDBStore` and `RocksDBKeyValueStore`-backed
...
### Behavioral changes
 ...
 -  `InvalidOperationException` and `InvalidTxException` now have a new
...
### Backward-incompatible API changes
 ...
 -  (Libplanet.Net) `ITransport` exception handling overhauled.  [[#1911]]
     -  `ITransport.SendMessageAsync()` now only throws
        `CommunicationException` for a normal failure of sending and
        receiving `Message`s; old exceptions such as `TimeoutException`,
        `InvalidMessageSignatureException`, etc. are attached to
        `InnerException` property to a thrown `CommunicationException`.
...
### Added APIs
 ...
 -  (Libplanet.Net) `InvalidCredentialException` class added.
    [[#1904], [#1905]]
 -  (Libplanet.Net) `CommunicationException` class added.  [[#1911]]
...
### Backward-incompatible API changes
 ...
 -  Added `Libplanet.Data.Store.PruneOutdatedChains(bool noopWithoutCanon)` method.
    [[#1874], [#1878]]
...
### Backward-incompatible storage format changes
 ...
 -  (Libplanet.Data.RocksDB) `RocksDBStore` became not to use [column families]
...
Released on March 15, 2022.
 ...
 -  (Libplanet.Data.RocksDB) `RocksDBStore.GetBlockDigest()` became to silently
...
### Backward-incompatible API changes
 ...
 -  The return type of `MerkleTrieExtensions.DifferentNodes()` became
    ...
    [[#1729]]
 -  (Libplanet.Commands) Replaced `string? passphrase = null` parameter
    of methods belonging `KeyCommand` and `ApvCommand` with
    `PassphraseParameters passphrase`.  [[#1593], [#1823]]
 -  (Libplanet.Commands) Replaced `KeyCommand.UnprotectKey(Guid keyId,
...
### Added APIs
 ...
 -  (Libplanet.Commands) Added `PassphraseParameters` class.
    [[#1593], [#1823]]
 -  (Libplanet.Commands) Added `KeyCommand.UnprotectKey(Guid keyId,
...
### Behavioral changes
 ...
 -  `BlockExecutor<T>.Evaluate()` method became to log message directly
...
### Dependencies
 ...
 -  (Libplanet.Commands) Upgraded *Cocona.Lite* from 1.5.\* to
...
Released on January 26, 2022.
 ...
 -  Fixed a bug where `BlockExecutor<T>`'s logs had been ignored due to
...
### Backward-incompatible API changes
 ...
 -  Removed `StateGetter<T> stateGetter` and `BalanceGetter<T> balanceGetter`
    parameters from `BlockExecutor<T>()` constructor.  [[#1703]]
 -  Added `IBlockChainStates<T> blockChainStates` parameter to
    `BlockExecutor<T>()` constructor.  [[#1703]]
 ...
 -  `nullable` context enabled for `IProtocol` interface and `Kademlia`,
    `Kademlia`, and `RoutingTable` classes.  [[#1692]]
...
### Bug fixes
 ...
 -  Fixed a bug where unnecessary additional attempts were made to
    unresponsive `Peer`s when discovering a `Peer` through `Kademlia`.
    [[#1692]]
...
Released on January 18, 2022.
 ...
 -  Some additional logging to track down issues with staging `Transaction<T>`s.
    [[#1718]]
 -  (Libplanet.Data.RocksDB) Fixed `RocksDBStore`'s inaccurate lock handling.
    [[#1719]]
...
### Behavioral changes
  ...
  -  (Libplanet.Data.RocksDB) Improved performance of
...
Released on December 13, 2021.
 ...
 -  (Libplanet.Data.RocksDB) Fixed a bug where `RocksDBStore.ForkBlockIndexes()`
...
### Added APIs
 ...
 -  Added `PreEvaluationBlock<T>.DetermineStateRootHash(BlockChain<T>,
    ...
    method.  [[#1636]]
 -  Added `PreEvaluationBlock<T>.DetermineStateRootHash(IAction?, TrieStateStore,
...
### Backward-incompatible API changes
 ...
 -  The type of `BlockPolicy<T>.GetMaxBlockBytes(long)` method became `long`
    (was `int`).  [[#1609]]
...
### Backward-incompatible API changes
 ...
 -  (Libplanet.Data.RocksDB) Removed `MonoRocksDBStore` class.
    [[#1513], [#1579]]
...
### Behavioral changes
 ...
 -  Default implementation `BlockPolicy<T>.ValidateNextBlock()`
    of `BlockPolicy<T>` now validates the type for `Block<T>.HashAlgorithm`,
...
### Deprecated APIs
 ...
 -  (Libplanet.Data.RocksDB) `MonoRocksDBStore` will be gone in the next minor
...
Released on October 29, 2021.
 ...
 -  `BlockHeader.MineNonce()` method and other mining methods based on it
...
### Backward-incompatible API changes
 ...
 -  Removed `Libplanet.Data.Store.StageTransactionIds()` method.  [[#1166], [#1535]]
 -  Removed `Libplanet.Data.Store.UnstageTransactionIds()` method.  [[#1166], [#1535]]
 -  Removed `Libplanet.Data.Store.IterateStagedTransactionIds()` method.  [[#1166], [#1535]]
 ...
 -  Removed unused `HashAlgorithmGetter` type parameter from
    `BlockExecutor<T>()` constructor.  [[#1537]]
...
### Added APIs
 ...
 -  Added
    `BlockHeader.MineNonce(HashAlgorithmType, int, CancellationToken)`
...
### Behavioral changes
 ...
 -  `BlockHeader.MineNonce()` method became to use multithreading when
    looking for the nonce.  [[#1546]]
 -  `Libplanet.Data.Store.ForkTxNonces()` method became to throw `ChainIdNotFoundException`
...
### Bug fixes
 ...
 -  Fixed `DefaultStore.ForkBlockIndexes()` method's bug that it had done
    ...
    been compliant with `Libplanet.Data.Store.ForkBlockIndexes()` method's specification.
    [[#1544]]
 -  Fixed `Swarm<T>`'s bug that it had thrown `TimeoutException` if any
    peer in configured `SwarmOptions.StaticPeers` is unreachable.
    [[#1550], [#1551]]
 -  (Libplanet.Data.RocksDB) Fixed `RocksDBStore.ForkBlockIndexes()` method's
    bug that it had done silently no-op when a given `sourceChainId` had not
    existed, which had not been compliant with `Libplanet.Data.Store.ForkBlockIndexes()`
    method's specification.  [[#1544]]
 -  (Libplanet.Data.RocksDB) Fixed `MonoRocksDBStore.ForkBlockIndexes()`
    method's bug that it had done silently no-op when a given `sourceChainId`
    had not existed, which had not been compliant with
    `Libplanet.Data.Store.ForkBlockIndexes()` method's specification.  [[#1544]]
...
Released on October 21, 2021.
 ...
 -  `StoreBase.GetBlock<T>()` method now throws `InvalidOperationException`
    when it fails to load a requested block's transactions.  [[#1500], [#1532]]
 -  (Libplanet.Data.RocksDB) Operations on `RocksDBStore` that update data
...
### Backward-incompatible API changes
 ...
 -  `Hashcash.Stamp(Nonce)` delegate's return type became `IEnumerable<byte[]>`
    (was `byte[]`).  [[#1492]]
 -  Moved `BlockDigest` struct to `Libplanet.Data` namespace (from
    `Libplanet.Blocks` namespace).  [[#1492]]
 -  Removed `Block<T>.ToBlockDigest()` method.  Use `BlockDigest.Create<T>()`
    static method instead.  [[#1492]]
 -  `BlockExecutor<T>.Evaluate()` method's `block` parameter became to take
 ...
 -  `Block<T>` and `BlockHeader` became aware of `HashAlgorithmType` used for
    proof-of-work mining.  [[#1492]]
     ...
     -  Added `HashAlgorithmGetter hashAlgorithmGetter` parameter to
        `Libplanet.Data.Store.GetBlock<T>()` method.
 ...
 -  `TrieStateStore` now requires implementations to be trie.
    [[#1128], [#1146], [#1492]]
     -  Added `TrieStateStore.GetStateRoot()` method.
     -  Added `TrieStateStore.PruneStates()` method.
     -  Removed `TrieStateStore.SetStates<T>()` method.
     -  Removed `TrieStateStore.GetState()` method.
     -  Removed `TrieStateStore.ContainsBlockStates()` method.
     -  Removed `TrieStateStore.ForkStates<T>()` method.
...
### Added APIs
 ...
 -  Added `BlockHeader` class.  [[#1164], [#1457], [#1492], [#1507]]
 ...
 -  Added `PreEvaluationBlock<T>` class.
    [[#1146], [#1164], [#1457], [#1492], [#1507]]
 -  Added `BlockDigest.Create<T>()` static method.  [[#1492]]
 ...
 -  Added `BlockHeader(PreEvaluationBlockHeader, HashDigest<SHA256>,
    ImmutableArray<byte>?, BlockHash)` overloaded constructor.
    [[#1164], [#1457], [#1492], [#1507]]
 -  Added `IBlockHeader` interface.  [[#1164], [#1457], [#1492], [#1507]]
     -  `Block<T>` became to implement `IBlockHeader` interface.
     -  `BlockHeader` became to implement `IBlockHeader` interface.
     -  `BlockHeader` became to implement `IBlockHeader` interface.
     -  `BlockContent<T>` became to implement `IBlockHeader` interface.
     -  `PreEvaluationBlockHeader` became to implement `IBlockHeader`
        interface.
     -  `PreEvaluationBlock<T>` became to implement `IBlockHeader` interface.
     -  `BlockDigest` became to implement `IBlockHeader` interface.
 ...
 -  Added `IPreEvaluationBlock<T>` interface.  [[#1164], [#1492]]
     ...
     -  `PreEvaluationBlock<T>` became to implement `IPreEvaluationBlock<T>`
        interface.
     -  `BlockExecutor<T>.Evaluate()` method's `block` parameter became to
 ...
 -  Added `BlockHeaderExtensions` static class.  [[#1164], [#1492]]
...
### Backward-incompatible API changes
 ...
 -  `BlockPolicy<T>.GetMaxBlockBytes()` description changed for `0`
    and negative values.  [[#1449], [#1463]]
     -  Returned values from these will now be taken literally
        by `BlockChain<T>`.
 -  Removed `BlockPolicy<T>.MaxTransactionsPerBlock` property.  It is replaced
    by `BlockPolicy<T>.GetMaxTransactionsPerBlock(long index)` method.
    [[#1447]]
 -  Added `BlockPolicy<T>.GetMaxTransactionsPerBlock(long index)` method.
    [[#1447]]
 -  Added `BlockPolicy<T>.GetMinTransactionsPerBlock(long index)` method.
    [[#1479]]
 -  Added `BlockPolicy<T>.GetMaxTransactionsPerSignerPerBlock(long index)`
    method.  [[#1449], [#1463]]
 ...
 -  `Libplanet.Data.Store`, `TrieStateStore`, and `IKeyValueStore` interfaces now inherit
    `IDisposable`.  [[#1448], [#1474]]
 -  Multiple changes for `BlockPolicy<T>` interface.  [[#1485]]
     -  `bool DoesTransactionFollowsPolicy(BlockChain<T>, Transaction<T>)`
        changed to `InvalidOperationException? ValidateNextBlockTx(
     ...
     -  `BlockPolicy<T>`, the default implementation for `BlockPolicy<T>`,
...
### Added APIs
 ...
 -  Added `BlockPolicyViolationException` and `InvalidOperationException`
    classes.  [[#1485]]
...
### Behavioral changes
 ...
 -  `StateCompleterSet<T>.Recalculate` now evaluates states even for those
    already in `TrieStateStore`.  Moreover, it also terminates early if possible
...
### Behavioral changes
 ...
 -  `Transaction<T>.Validate()` became to throw `InvalidOperationException`
...
### Backward-incompatible API changes
 ...
 -  Block hashes are now represented as `BlockHash`, which was introduced in
    ...
    [[#1192], [#1197]]
     ...
     -  The type of `InvalidTxGenesisHashException.ImproperGenesisHash` property
        became `BlockHash?` (was `HashDigest<SHA256>?`).
     -  The return type of `Libplanet.Data.Store.IndexBlockHash()` method became `BlockHash?`
        (was `HashDigest<SHA256>?`).
     -  The type of `Libplanet.Data.Store.AppendIndex()` method's `hash` parameter became
        `BlockHash` (was `HashDigest<SHA256>`).
     -  Replaced `Libplanet.Data.Store.ForkBlockIndexes()` method's
        `HashDigest<SHA256> branchPoint` parameter with `BlockHash branchpoint`.
     -  The return type of `Libplanet.Data.Store.IterateIndexes()` method became
        `IEnumerable<BlockHash>` (was `IEnumerable<HashDigest<SHA256>>`).
     -  The return type of `Libplanet.Data.Store.IterateBlockHashes()` method became
        `IEnumerable<BlockHash>` (was `IEnumerable<HashDigest<SHA256>>`).
     -  The type of `Libplanet.Data.Store.GetBlock<T>()` method's `blockHash` parameter
        became `BlockHash` (was `HashDigest<SHA256>`).
     -  The type of `Libplanet.Data.Store.GetBlockIndex()` method's `blockHash` parameter
        became `BlockHash` (was `HashDigest<SHA256>`).
     -  The type of `Libplanet.Data.Store.GetBlockDigest()` method's `blockHash` parameter
        became `BlockHash` (was `HashDigest<SHA256>`).
     -  The type of `Libplanet.Data.Store.DeleteBlock()` method's `blockHash` parameter
        became `BlockHash` (was `HashDigest<SHA256>`).
     -  The type of `Libplanet.Data.Store.ContainsBlock()` method's `blockHash` parameter
        became `BlockHash` (was `HashDigest<SHA256>`).
     -  The type of `TrieStateStore.GetState()` method's `blockHash` parameter
        became `BlockHash` (was `HashDigest<SHA256>`).
     -  The type of `TrieStateStore.ContainsBlockStates()` method's `blockHash`
        parameter became `BlockHash` (was `HashDigest<SHA256>`).
 ...
 -  Hash algorithm for <abbr title="proof-of-work">PoW</abbr> (Hashcash) became
    configurable.  [#1314], [#1352]
     -  Added `BlockPolicy<T>.GetHashAlgorithm()` method.
 ...
 -  Added `Libplanet.Data.Store.PutTxExecution(TxSuccess)` method.  [[#1156], [#1289]]
 -  Added `Libplanet.Data.Store.PutTxExecution(TxFailure)` method.  [[#1156], [#1289]]
 -  Added `Libplanet.Data.Store.GetTxExecution()` method.  [[#1156], [#1289]]
 -  Removed the optional parameter `Guid? chainId = null` from
    `TrieStateStore.GetState()` method.  [[#1289]]
 ...
 -  All methods pertaining to evaluating `IAction`s are moved
    to a new `BlockExecutor` class.  [[#1301], [#1305]]
     -  Removed `Block<T>.Evaluate()` method.
        Use `BlockExecutor.Evaluate()` method instead.
 ...
 -  Added `Libplanet.Data.Store.BlockHashByTxId.Add(TxId, BlockHash)` method.
    [[#1294], [#1328]]
 -  Added `Libplanet.Data.Store.BlockHashByTxId[TxId]` method.
    [[#1294], [#1328]]
 -  Added `Libplanet.Data.Store.BlockHashByTxId.Remove(TxId, BlockHash)` method.
    [[#1294], [#1328]]
 -  Added `Libplanet.Data.Store.IterateTxIdBlockHashIndex(TxId)` method.  [[#1294], [#1328]]
...
### Added APIs
 ...
 -  Added `BlockExecutor` class.  [[#1301], [#1305]]
 ...
 -  Added `InvalidOperationException` class.  [[#1242]]
...
### Behavioral changes
 ...
 -  `ITransport.StartAsync()` and `ITransport.RunAsync()` became to throw
    `InvalidOperationException` instead of `SwarmException`.  [[#1242]]
 -  `NetMQTransport` became to enforce NetMQ/[AsyncIO] to use its pure .NET
    ...
    [[#247], [#1278]]
 -  (Libplanet.Data.RocksDB) `RocksDBStore.ForkBlockIndexes()` became to share
 ...
 -  `BlockChain<T>.Append()` cumulates indexes for pairs (TxId and BlockHash).
    ...
    [[#1315], [#1329]]
 -  `BlockExecutor<T>.EvaluateActions()` now throws an unmanaged exception
...
### Bug fixes
 ...
 -  Fixed a bug where executing `Transaction<T>.Actions` had not been atomic.
    ...
    [[#1267], [#1275]]
     ...
     -  State-wise, `Transaction<T>`s having any exception-throwing action
        now do not commit any changes at all to `TrieStateStore`.
 ...
 -  Fixed a bug where `Kademlia.BootstrapAsync()` has sent multiple
    `Ping` messages to other peers.  [[#1219]]
 -  Fixed a bug where `Kademlia.CheckReplacementCacheAsync()` has
 ...
 -  (Libplanet.Data.RocksDB) Fixed a bug where `RocksDBStore.Dispose()`,
 ...
 -  Fixed a bug where `DefaultStore.ForkBlockIndexes()` hadn't copied genesis
    block.  [[#1325]]
 -  (Libplanet.Data.RocksDB) Fixed a bug where `RocksDBStore.GetBlock<T>()`
...
### Backward-incompatible API changes
 ...
 -  Added `BlockPolicy<T>.CanonicalChainComparer` property to make
 ...
 -  Added `reorgResistantHeight` parameter into `DelayedActionRenderer<T>()`
    constructor.  [[#1163]]
 -  Added `Libplanet.Data.Store.SetBlockPerceivedTime()` method.  [[#1184]]
 -  Added `Libplanet.Data.Store.GetBlockPerceivedTime()` method.  [[#1184]]
 ...
 -  Added `Libplanet.Data.Store.ForkTxNonces()` method.  [[#1198]]
...
### Backward-incompatible storage format changes
 ...
 -  (Libplanet.Data.RocksDB) The blocks and transactions became stored in
 ...
 -  (Libplanet.Data.RocksDB) Continue on partitioning of database,
...
### Added APIs
 ...
 -  Added `BlockChain<T>.StagedTransactions.Keys` method.  [[#1089]]
 -  (Libplanet.Data.RocksDB) Added `maxTotalWalSize`, `keepLogFileNum` and
 ...
 -  Added `TurnClient.ReconnectTurn(CancellationToken)` method.
    [[#1074], [#1102]]
 -  Added `Libplanet.Renderers.Debug.RenderRecord<T>` abstract class
 ...
 -  Added `Libplanet.Renderers.Debug.RecordingActionRenderer<T>`
    class.  [[#1119]]
 -  Added `Libplanet.Renderers.Debug.ValidatingActionRenderer<T>`
    class.  [[#1119]]
 -  Added `Libplanet.Renderers.Debug.InvalidRenderException<T>`
    class.  [[#1119]]
 ...
 -  Added `Kademlia` class which implements `IProtocol`.
    [[#1120], [#1135]]
...
### Behavioral changes
 ...
 -  `Swarm<T>` became not to call `Kademlia.RebuildConnectionAsync()`
...
Released on November 25, 2020.
 ...
 -  Fixed `BlockChain<T>.Append()` method's bug that it had accepted
    ...
    the `BlockPolicy<T>.MaxTransactionsPerBlock` property.  Now it throws
...
### Backward-incompatible API changes
 ...
 -  Extracted `Libplanet.Data.Store`'s some methods dedicated to block states into
 ...
 -  The signature of `Libplanet.Data.Store.LookupStateReference<T>(Guid, string, Block<T>)`
    method was changed to `LookupStateReference(Guid, string, long)`.  [[#950]]
 -  Added `TrieStateStore`-typed `stateStore` to `BlockChain<T>` constructor.
    [[#950]]
 ...
 -  Added `BlockPolicy<T>.MaxTransactionsPerBlock` property.
    [[#1037], [#1050]]
 -  Added `BlockPolicy<T>.GetMaxBlockBytes()` method.  [[#201], [#1050]]
 -  `BlockPolicy<T>.DoesTransactionFollowPolicy()` method became to take
...
### Added APIs
 ...
 -  Added `TrieStateStore` interface.  [[#950]]
 ...
 -  (Libplanet.Data.RocksDB) `RocksDBStore` became to implement
    `IBlockStatesStore`.  [[#950]]
 -  (Libplanet.Data.RocksDB) Added `RocksDBKeyValueStore`.  [[#939]]
...
### Behavioral changes
 ...
 -  `BlockChain<T>.Append()` method became to throw
    ...
    `BlockPolicy.GetMaxBlockBytes()`.  [[#201], [#1050]]
 -  `BlockChain<T>.MineBlock()` method became to cut off transactions to include
    to fit into the limitation configured by `BlockPolicy.GetMaxBlockBytes()`.
    [[#201], [#1050]]
 ...
 -  `BlockChain<T>` instead of `BlockPolicy<T>` became to validate `Block<T>`s
    to append so that even if an empty implementation of `BlockPolicy<T>` is
...
Released on June 2, 2020.
 ...
 -  (Libplanet.Data.RocksDB) Fixed a bug that  `RocksDBStore.DeleteChainId()`
    method had thrown `KeyNotFoundException` when there's no such chain ID.
    [[#891]]
 -  (Libplanet.Data.RocksDB) Fixed a bug that `RocksDBStore` had written logs
...
Released on May 20, 2020.
 ...
 -  (Libplanet.Data.RocksDB) Fixed a memory leak bug in `RocksDBStore`.
    [[#870]]
...
### Backward-incompatible API changes
 ...
 -  `StoreBase` class became to implement `IDisposable`.  [[#789]]
 -  Removed `Libplanet.Data.Store.DeleteIndex(Guid, HashDigest<SHA256>)` method.  [[#802]]
 ...
 -  Replaced `BlockChain<T>.UnstageTransactions()` with `.UnstageTransaction()`
    that receives only one transaction.  [[#820]]
 -  Added `BlockPolicy.DoesTransactionFollowPolicy()` method which is a method
...
### Added APIs
 ...
 -  Added `Libplanet.Data.Store.GetBlockDigest(HashDigest<SHA256>)` method.  [[#785]]
 -  Added `Block<T>.ToBlockDigest()` method.  [[#785]]
 -  Added `ByteArrayExtensions` class.  [[#803]]
 -  Added `Libplanet.Data.Store.PruneBlockStates<T>(Guid, Block<T>)` method.  [[#790]]
...
### Backward-incompatible API changes
 ...
 -  The internal representation for state keys became `string` (was `Address`).
    [[#368], [#774]]
     -  The return type of `Libplanet.Data.Store.GetBlockStates()` method became
        `IImmutableDictionary<string, IValue>` (was `AddressStateMap`,
        which was removed too).  [[#368], [#774]]
     -  The type of the second parameter of `Libplanet.Data.Store.SetBlockStates()` method
        became `IImmutableDictionary<string, IValue>` (was `AddressStateMap`,
        which was removed too).  [[#368], [#774]]
     -  The second parameter of `Libplanet.Data.Store.IterateStateReferences()` method became
        `string key` (was `Address address`).  [[#368], [#774]]
     -  The second parameter of `Libplanet.Data.Store.StoreStateReference()` method became
        `IImmutableSet<string> keys` (was `IImmutableSet<Address> addresses`).
        [[#368], [#774]]
     -  `Libplanet.Data.Store.ListAddresses()` method was replaced by `Libplanet.Data.Store.ListStateKeys()`
        method.  [[#368], [#774]]
 ...
 -  Removed `BlockChain<T>.Contains(TxId)` method.
    Use `Libplanet.Data.Store.ContainsTransaction(TxId)` instead.  [[#676]]
 ...
 -  Added `Libplanet.Data.Store.ContainsBlock(HashDigest<SHA256>)` method.  [[#678]]
 -  Removed `AddressStateMap` class.  [[#98], [#368], [#692], [#774]]
     -  The return type of `BlockChain<T>.GetState()` method became `IValue`
        (was `AddressStateMap`).
     -  The return type of `Libplanet.Data.Store.GetBlockStates()` method became
        `IImmutableDictionary<string, IValue>` (was `AddressStateMap`).
     -  `Libplanet.Data.Store.SetBlockStates()` method became to take
 ...
 -  `StoreExtension.LookupStateReference<T>()` method was replaced by
    `Libplanet.Data.Store.LookupStateReference<T>()` method.  [[#722], [#774]]
...
### Added APIs
 ...
 -  Added `Libplanet.Data.Store.ListAllStateReferences<T>()` method.  [[#701], [#703]]
 -  Added `Libplanet.Data.Store.ListStateKeys()` method to replace `Libplanet.Data.Store.ListAddresses()`
    method.  [[#368], [#774]]
 -  Added `Libplanet.Data.Store.LookupStateReference<T>()` method to replace
 ...
 -  Added `StoreExtension.Copy(this Libplanet.Data.Store, Libplanet.Data.Store)` extension method.  [[#753]]
...
### Backward-incompatible interface changes
 ...
 -  Renamed `Libplanet.Data.Store.IterateIndex()` method to `IterateIndexes()`.
    [[#462], [#560]]
 -  `Swarm<T>` class became to implement `IDisposable` again and should be
    disposed to clean up its internal resources.  [[#485]]
 -  `Libplanet.Data.Store.IterateStateReferences()` method became to receive
 ...
 -  `BlockChain<T>` does not implement `IReadOnlyList<T>` anymore.  [[#630]]
     ...
     -  `BlockPolicy<T>.GetNextBlockDifficulty()` method became to receive
        `BlockChain<T>` instead of `IReadOnlyList<Block<T>>`.  [[#630]]
     -  `BlockPolicy<T>.ValidateNextBlock()` method became to receive
        `BlockChain<T>` instead of `IReadOnlyList<Block<T>>`.  [[#630]]
...
### Behavioral changes
 ...
 -  `Libplanet.Data.Store.PutBlock<T>()` became to do nothing when it takes
...
### Backward-incompatible interface changes
 ...
 -  The concept of "namespaces" in `Libplanet.Data.Store` was replaced by "chain IDs"
    to be consistent with `BlockChain<T>`.  [[#483], [#486]]
     -  Renamed `Libplanet.Data.Store.ListNamespaces()` method to `ListChainIds()`.
     -  Renamed `Libplanet.Data.Store.DeleteNamespace()` method to `DeleteChainId()`.
     -  Renamed `Libplanet.Data.Store.GetCanonicalNamespace()` method to
        `GetCanonicalChainId()`.
     -  Renamed `Libplanet.Data.Store.SetCanonicalNamespace(string)` method to
        `SetCanonicalChainId(Guid)`.
     -  Replaced `namespace`/`sourceNamespace`/`destinationNamespace` parameters
        taking `string` of methods in `Libplanet.Data.Store` and `StoreExtension` with
 ...
 -  `Libplanet.Data.Store.StoreStateReference<T>(string, IImmutableSet<Address>, Block<T>)`
 ...
 -  Added `Libplanet.Data.Store.ForkBlockIndexes()` method.  [[#420]]
 -  Removed `addressesToStrip` parameter from `Libplanet.Data.Store.ForkStateReferences<T>()`
 ...
 -  Removed the concept of "staged transactions that should not be broadcasted,"
    ...
    `BlockPolicy<T>.BlockAction` property which was introduced at 0.5.0.
    All staged transactions became broadcasted.  [[#319], [#470]]
     ...
     -  Removed `toBroadcast` parameter from
        `Libplanet.Data.Store.IterateStagedTransactionIds(bool)` method.
     -  `Libplanet.Data.Store.StageTransactionIds(IDictionary<TxId, bool>)` method became
...
### Bug fixes
 ...
 -  Fixed a bug that `Swarm<T>.PreloadAsync()` method had thrown `LiteException`
    (or other exception depending on `Libplanet.Data.Store`), which indicates a state
 ...
 -  Fixed a bug tag `Swarm<T>.StartAsync()` sometimes had thrown an exception
    from `Libplanet.Data.Store` (e.g., `NullReferenceException`) during broadcasting
...
### Backward-incompatible interface changes
 ...
 -  Added `Libplanet.Data.Store.GetBlockIndex()` method.  [[#385]]
 ...
 -  Added `BlockPolicy<T>.BlockAction` property.  [[#319], [#367]]
 ...
 -  `BlockDownloadState.TotalBlockCount` and
    ...
    [[#396], [#399]]
 -  `Libplanet.Data.Store.IterateIndex()` method became to receive `offset` and `limit`
    parameters.  [[#425]]
 -  Added `Libplanet.Data.Store.GetCanonicalNamespace()` method.  [[#426]]
 -  Added `Libplanet.Data.Store.SetCanonicalNamespace()` method.  [[#426]]
...
### Added interfaces
 ...
 -  Added `trustedStateValidators` option to `Swarm<T>.PreloadAsync()` method.
    ...
    for themselves.
    [[#272], [#343]]
 -  Added `StoreExtension.ListAllStateReferences(this Libplanet.Data.Store, string,
...
### Behavioral changes
 ...
 -  `BlockChain<T>` became to evaluate `BlockPolicy<T>.BlockAction` and set the
 ...
 -  `FileStore` and `LiteDBStore` became to guarantee atomicity of storing
    transactions.  [[#413]]
 -  `Libplanet.Data.Store.PutTransaction<T>()` became to do nothing when it takes
...
### Backward-incompatible interface changes
 ...
 -  `Peer.AppProtocolVersion` became nullable to represent `Peer` whose version
    is unknown.  [[#280]]
 -  Added `Libplanet.Data.Store.ListAddresses()` method.  [[#272], [#285]]
 -  Added `Libplanet.Data.Store.ListTxNonces()` method.  [[#272], [#309], [#310]]
 ...
 -  `BlockChain<T>.StageTransactions` became to receive
    ...
    [[#274], [#297]]
 -  `Libplanet.Data.Store.StageTransactionIds()` method became to receive
    `IDictionary<TxId, bool>` instead of `ISet<TxId>`.  [[#274], [#297]]
 -  `Libplanet.Data.Store.IterateStagedTransactionIds()` method became to receive
 ...
 -  `Libplanet.Data.Store` became to treat a "tx nonce" mere a `long` integer instead of
    a stack of block hashes.  [[#272], [#307], [#309], [#310]]
     -  `Libplanet.Data.Store.IncreaseTxNonce<T>(string, Block<T>)` method was replaced by
        `Libplanet.Data.Store.IncreaseTxNonce(string, Address, long)` method.
     -  Removed `Libplanet.Data.Store.ForkTxNonce()` method.
     -  `FileStore` became to occupy fewer bytes for storing tx nonces.
        This change broke file-level backward compatibility.
 -  `Libplanet.Data.Store` became possible to look up multiple state references in a stack.
    [[#272], [#307]]
     -  Removed `Libplanet.Data.Store.LookupStateReference<T>()` method.
        Instead, a newly added static class `StoreExtension` provides
        an extension method of the same name.
     -  Added `Libplanet.Data.Store.IterateStateReferences()` method.
 ...
 -  Added `Libplanet.Data.Store.DeleteNamespace()` method.  [[#329]]
...
### Behavioral changes
 ...
 -  `Libplanet.Data.Store.GetBlockStates()` became able to return `null` to represent
...
### Backward-incompatible interface changes
 ...
 -  `BlockPolicy<T>.GetNextBlockDifficulty()` method became to receive
    `IReadOnlyList<Block<T>>` instead of `IEnumerable<Block<T>>`.  [[#205]]
 -  Added
    `BlockPolicy<T>.ValidateNextBlock(IReadOnlyList<Block<T>>, Block<T>)`
    method.  [[#210]]
 -  Removed `BlockPolicy<T>.ValidateBlocks()` method.  [[#210]]
 ...
 -  The signature of `Libplanet.Data.Store.PutBlock<T>(Block<T>)` method was changed to
 ...
 -  `Block<T>.Hash` is no longer calculated using the full data of the
    ...
    [[#234]]
 -  Added `Libplanet.Data.Store.LookupStateReference<T>(string, Address, Block<T>)` method.
    [[#232]]
 -  Added `Libplanet.Data.Store.StoreStateReference<T>(string, Block<T>)` method.
    [[#232]]
 -  Added `Libplanet.Data.Store.ForkStateReferences<T>(string, string, Block<T>,
 ...
 -  Added `Transaction<T>.Nonce` and `RawTransaction.Nonce` properties.
    [[#246]]
 -  Added `Libplanet.Data.Store.GetTxNonce(string, Address)` method.  [[#246]]
 -  Added `Libplanet.Data.Store.IncreaseTxNonce<T>(string, Block<T>)` method.  [[#246]]
 -  Added `Libplanet.Data.Store.ForkTxNonce<T>(string, string, Block<T>,
...
### Added interfaces
 ...
 -  Added `BlockDownloadState` class to represent a block downloading state.
    [[#204]], [[#206]]
 -  Added `BlockPolicyExtension.ValidateBlocks<T>(BlockPolicy<T>,
 ...
 -  Made `InvalidBlockTimestampException` class `public` so that it can be
    caught.  [[#133], [#251]]
 -  Added `InvalidOperationException` class.  [[#246]]
...
### Behavioral changes
 ...
 -  `BlockChain<T>.Append()` method became to throw `InvalidOperationException`
...
Released on April 5, 2019.
 ...
 -  Removed `AddressTransactionSet` which handles handle `Address` to
    `IEnumerable<TxId>` indices, and the following methods in `Libplanet.Data.Store`:
     -  `Libplanet.Data.Store.IterateAddresses()`
     -  `Libplanet.Data.Store.GetAddressTransactionIds()`
     -  `Libplanet.Data.Store.AppendAddressTransactionId()`
     -  `Libplanet.Data.Store.CountAddresses()`
 -  Added `Libplanet.Data.Store.ListNamespaces()` method.
 -  `Libplanet.Data.Store.CountBlocks()` and `Libplanet.Data.Store.CountTransactions()` became to return
    `long`.
 -  Block/tx-related methods in `Libplanet.Data.Store` and `BaseIndex<T>` no longer
