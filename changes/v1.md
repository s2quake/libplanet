...
### Backward-incompatible API changes
 ...
 -  Changed `new BlockChain()` to accept `IBlockExecutor` interface
    instead of `BlockExecutor` object.  [[#3195]]
...
### Backward-incompatible API changes
 ...
 -  Added `SingleActionLoader`, `TypedActionLoader`, and `IndexedActionLoader`
    classes.  [[#3148]]
 -  Removed `trieGetter` parameter from `BlockExecutor` constructor.
    [[#3149]]
 -  Added `IBlockChainStates.GetTrie()` interface method.  [[#3149]]
 -  `BlockChain<T>.Create()` static method now requires `blockExecutor`
    parameter explicitly.  [[#3149]]
 -  Removed `BlockPolicy.NativeTokens` property.  [[#3153]]
     - Removed `NonNativeTokenException` class.
     - Removed `IActionContext.IsNativeToken()` method.
     - Removed `nativeTokenPredicate` parameter from `BlockExecutor()`
 ...
 -  Removed `genesisHash` parameter from `BlockExecutor()`.  [[#3164]]
...
{)` method no more fills `Transaction.UpdatedAddresses`
...
### Backward-incompatible API changes
 ...
 -  Changed return types and parameter types of several methods from
    `IReadOnlyList<ActionEvaluation>` to `IReadOnlyList<ActionEvaluation>`.
    [[#3089]]
 ...
 -  Added `IBlockExecutor.IActionLoader` property.  [[#3136]]
...
### Added APIs
 ...
 -  Added `IBlockExecutor` interface.  [[#3082]]
 ...
 -  Added `ActionEvaluation` interface.  [[#3089]]
...
Released on May 4, 2023.
 ...
 -  (Libplanet.Commands.Executable) The `planet` command now falls back to the default
...
### Backward-incompatible API changes
 ...
 -  Removed `PreEvaluationBlock<T>.Evaluate(PrivateKey, IAction?,
    Predicate<Currency>, TrieStateStore)`,
    `PreEvaluationBlock<T>.DetermineStateRootHash(IAction?, Predicate<Currency>,
    TrieStateStore)`, `PreEvaluationBlock<T>.DetermineStateRootHash(IAction?,
    Predicate<Currency>, TrieStateStore,
 ...
 -  (Libplanet.Commands) Changed signature of
    `ApvCommand.Verify(string?, string[]?, bool)` method to
    `ApvCommand.Verify(string?, PublicKey[]?, bool)`.  [[#3044]]
 -  Removed `PreEvaluationBlock<T>.Mine()` and `BlockHeader.MineNonce()`
    methods.  [[#3067]]
...
### Added APIs
 ...
 -  (@planetarium/account-web3-secret-storage) Added `Web3KeyMetadata`
    interface.  [[#3084]]
 -  (Libplanet.Commands.Executable) Added `-v`/`--validator-key` option to
...
### CLI tools
 ...
 -  (Libplanet.Commands.Executable) Now `planet` uses *libsecp256k1* (which is the same
