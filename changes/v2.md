...
### Backward-incompatible API changes
 ...
 -  Removed `BlockPolicy.FeeCalculator` interface property.  [[#3283]]
 ...
 -  Removed `IActionContext.Logs` property and `IActionContext.PutLog()` method.
    [[#3292]]
 -  Removed `ActionEvaluation.Logs` property. [[#3292]]
...
### Added APIs
 ...
 -  Changed the type for `ActionEvaluation.Logs` to
...
### Behavioral changes
 ...
 -  Changed `BlockExecutor` to evaluate all `IAction`s in a `Transaction`
...
Released on July 14, 2023.
 ...
 -  (Libplanet.Data.RocksDB) Slight improvement for speed and memory usage.
    [[#3298]]
...
### Backward-incompatible API changes
 ...
 -  `Vote.BlockHash` property became `BlockHash` type. (was `BlockHash?`)
    [[#3249]]
 -  `VoteMetadata(long, int, BlockHash?, DateTimeOffset, PublicKey, VoteType)`
    constructor became
    `VoteMetadata(long, int, BlockHash, DateTimeOffset, PublicKey, VoteType)`
    [[#3249]]
...
### Added APIs
 ...
 -  Changed `ActionEvaluation.OutputStates` to `ActionEvaluation.OutputState`.
    [[#3259]]
