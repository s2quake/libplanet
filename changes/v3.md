...
Released on January 18, 2024.
 ...
 -  (Libplanet.Store) Changed `TrieStateStore.Commit()` to return an `ITrie`
    with either its `Root` as `null` or a `HashNode`.  [[#3610]]
 -  (Libplanet.Store) Removed `TrieStateStore.PruneStates()` method.
    [[#3613], [#3614]]
...
`Transaction`s that aren't compatible with the updated specification in [#3567].
 ...
 -  (Libplanet.Action) Added `Account.RemoveState()` interface method.
    [[#3577]]
...
### Backward-incompatible API changes
 ...
 -  (Libplanet.Action) Changed `Account` to no longer track `IAccountDelta`.
    [[#3520]]
 ...
 -  (Libplanet.Action) Removed `Account.Delta` interface property.  [[#3535]]
 -  (Libplanet.Action) Changed constructor `Account(IAccountState,
...
### Backward-incompatible API changes
 ...
 -  All properties of `TxExecution` except `BlockHash` and `TxId` were
    overhauled.  [[#3429]]
 -  (Libplanet.Store) Removed `Libplanet.Store.Store.PutTxExecution(TxSuccess)` and
    `Libplanet.Store.Store.PutTxExecution(TxFailure)`;
    added `Libplanet.Store.Store.PutTxExecution(TxExecution)`.  [[#3429]]
...
### Backward-incompatible API changes
 ...
 -  (Libplanet.Store) Added `TrieStateStore.Commit()` method.  [[#3398]]
...
### Backward-incompatible API changes
 ...
 -  (Libplanet.Action) Renamed `IAccountStateDelta` as `Account`.
    [[#3337]]
