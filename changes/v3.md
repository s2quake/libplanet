...
Released on December 15, 2023.
 ...
 -  (Libplanet.Data.RocksDB) Added `DBOptions` type parameter to
...
### Backward-incompatible API changes
 ...
 -  (Libplanet.State) Removed `hashedSignature` of type `byte[]` parameter
    from `BlockExecutor.GenerateRandomSeed()`.  [[#3523]]
 ...
 -  (Libplanet.State) Changed constructor `Account(IAccountState,
...
### Backward-incompatible storage format changes
  ...
  -  (Libplanet.Data) Changed `Libplanet.Data.RocksDB` to use
     [`RocksDb`] instead of [`RocksDBSharp`].
     *Note* Cannot read new version of `Libplanet.Data.RocksDB`
     storage from under `Libplanet.Data.RocksDB` version 3.6.1.
     [[#1848], [#3487]]

### Added APIs

 -  (Libplanet.Data.RocksDB) Added `RocksDBInstanceType` enum.  [[#3488]]
 -  (Libplanet.Data.RocksDB) Changed `RocksDBStore` and `RocksDBKeyValueStore`
...
### Backward-incompatible API changes
 ...
 -  Changed `IBlockExecutor.Evaluate()`'s return type to
    `IReadOnlyList<CommittedActionEvaluation>` from
    `IReadOnlyList<ActionEvaluation>`.  [[#3445]]
 -  Changed `BlockChain.DetermineStateRootHash(IBlockExecutor,
    IPreEvaluationBlock, out IReadOnlyList<ActionEvaluation>)` to
    `BlockChain.DetermineStateRootHash(IBlockExecutor,
...
### Behavioral changes
 ...
 -  `IBlockExecutor.Evaluate()`, `BlockChain.EvaluateGenesis()`,
...
### Backward-incompatible API changes
 ...
 -  Changed `IBlockExecutor.Evaluate(IPreEvaluationBlock)` to
    `IBlockExecutor.Evaluate(IPreEvaluationBlock, HashDigest<SHA256>)`.
    [[#3438]]
 -  Changed `BlockExecutor` to accept `TrieStateStore` instead of
...
### Backward-incompatible API changes
 ...
 -  (Libplanet.Data) Removed `ITrie.Commit()` method.  [[#3392]]
...
### Added APIs
 ...
 -  (Libplanet.Data.RocksDB) `RocksDBStore` and `RocksDBKeyValueStore` became to
...
### Backward-incompatible API changes
 ...
 -  Changed `AccountDeltaExtensions.ToRawDelta()` to return
    `IImmutableDictionary<KeyBytes, IValue>` instead of
    `IImmutableDictionary<string, IValue>`.  [[#3321]]
 -  Changed `BlockExecutorExtensions.GetRawTotalDelta()` to return
    `IImmutableDictionary<KeyBytes, IValue>` instead of
    `IImmutableDictionary<string, IValue>`.  [[#3321]]
