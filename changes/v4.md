...
`IBlockExecutor`, which is `BlockExecutor`, no longer supports
...
### Deprecated APIs
 ...
 -  (Libplanet.Types) Removed `Nonce` struct.  [[#3793], [#3794]]
 -  Removed `AtomicActionRenderer` class.  [[#3795]]

### Backward-incompatible API changes

 -  (Libplanet.State) Changed `ActionEvaluate.Evaluate()` to no longer
    accept `IPreEvaluationBlock` with a protocol version less than
    `BlockHeader.PBFTProtocolVersion`.  [[#3789]]
 -  (Libplanet.State) Changed the description of `IActionEvaluate.Evaluate()`
    ...
    certain `BlockHeader.ProtocolVersion`s.  [[#3789]]
 -  (Libplanet.Types) Removed `nonce` parameter from
    `BlockHeader.DerivePreEvaluationHash()` and
    `BlockHeader.MakeCandidateData()` methods.  [[#3793], [#3794]]
...
### Added APIs
 ...
 -  (Libplanet.State) Added `BlockProtocolVersionNotSupportedException` class.
    [[#3789]]
...
### Backward-incompatible API changes
 ...
 -  (Libplanet.Types) Updated `BlockHeader.CurrentProtocolVersion`
    from 6 to 7.  [[#3769]]
 -  (Libplanet.Data) Added `IterateSubTrieValues(KeyBytes)` and
    `IterateSubTrieNodes(KeyBytes)` methods to `MerkleTrie`.  [[#3774]]
 -  (Libplanet.Types) Added `BlockHeader.CurrencyAccountProtocolVersion`.
    [[#3775]]
 -  (Libplanet.Mocks) Removed `MockWorldState.ToModern()` and
    `MockWorldState.SetTotalSupply()` methods.  [[#3778]]
 -  (Libplanet.State) Removed `TotalSupplyNotTrackableException` class.
    [[#3780]]
 -  (Libplanet.State) `IWorldState.GetTotalSupply()` no longer throws
    a `TotalSupplyNotTrackableException` but returns a zero amount of
    corresponding `FungibleAssetValue`.  [[#3780]]
 -  (Libplanet.State) Changed the precednce for the types of `Exception`s
    that may be thrown by `World.MintAsset()` and
    `World.BurnAsset()`.

### Added APIs

 -  (Libplanet.State) Added `CurrencyAccount` class.  [[#3779]]
...
### Backward-incompatible API changes
 ...
 -  (Libplanet.State) Removed `World.SetValidator()` extension method.
    Use `World.SetValidatorSet()` extension method instead.  [[#3735]]
 -  (Libplanet.Types) Renamed `BlockHeader.PoWProtocolVersion` to
    `BlockHeader.PBFTProtocolVersion` and `BlockHeader.LegacyStateVersion`
    to `BlockHeader.WorldStateProtocolVersion` while increasing each value
    by 1.  [[#3736]]
 -  (Libplanet.Data) Changed the type of `TrieMetadata.Version` from
    `BigInteger` to `int`.  [[#3738]]
 -  (Libplanet.Data) Changed `TrieMetadata` to throw an `ArgumentException`
    when trying to create an instance with an invalid version.  [[#3738]]
 -  (Libplanet.State) Added `IWorldState.Version` interface property.
    [[#3739]]
 -  (Libplanet.Types) Updated `BlockHeader.CurrentProtocolVersion`
    from 5 to 6.  [[#3741]]
 -  (Libplanet.Types) Added `BlockHeader.TransferFixProtocolVersion`,
    `BlockHeader.SignatureProtocolVersion`, and
    `BlockHeader.TransactionOrderingFixProtocolVersion` constants.  [[#3742]]
 -  (Libplanet.State) Removed `ReservedAddresses.FungibleAssetAccount`.
    [[#3745]]
 -  (Libplanet.State) Changed `ReservedAddresses.ValidatorSetAccount`'s value
    from `0x1000000000000000000000000000000000000002`
    to `0x100000000000000000000000000000000000001`.  [[#3745]]

### Added APIs

 -  (Libplanet.State) Added `ValidatorSetAccount` class.  [[#3745]]
...
### Backward-incompatible API changes
 ...
 -  (Libplanet.State) Added `Txs` property of
    type `IReadOnlyList<ITransaction>?` to `IActionContext`.  [[#3713]]
 -  (Libplanet.State) Removed `TotalFungibleAssets` property from
    `World`.  [[#3714]]
 -  (Libplanet.State) Changed `GetBalance()`, `GetTotalSupply()`, and
    `GetValidatorSet()` of `IWorldState` to extension methods.  [[#3715]]
 -  (Libplanet.State) Changed `MintAsset()`, `BurnAsset()`, `TransferAsset()`,
    and `SetValidator()` of `World` to extension methods.  [[#3715]]
 -  (Libplanet.State) Removed `allowNegativeBalance` parameter from
    `World.TransferAsset()` extension method.  [[#3725], [#3728]]
 -  (Libplanet.Data) Removed `journal`, `indexCacheSize`, and `flush`
...
### Backward-incompatible API changes
 ...
 -  (Libplanet.State) Moved `GetBalance()` and `GetTotalSupply()` methods from
    `IAccountState` to `IWorldState`.  [[#3694], [#3697]]
 -  (Libplanet.State) Moved `MintAsset()`, `BurnAsset()`, and `TransferAsset()`
    methods from `Account` to `World`.  [[#3694], [#3697]]
 -  (Libplanet.State) Removed `TotalSupplyDiff`, `FungibleAssetValueDiff`,
    and `ValidatorDiff` properties from `AccountDiff`.  [[#3694], [#3697]]
 -  (Libplanet.State) Removed `Uncommitted` property and `CommitAccount()`
    method from `IWorldDelta`.  [[#3694], [#3699]]
 -  (Libplanet.State) Moved `GetValidatorSet()` from `IAccountState`
    to `IWorldState`.  [[#3702]]
 -  (Libplanet.State) Moved `SetValidator()` from `Account` to `World`.
    [[#3702]]
...
### Backward-incompatible API changes
 ...
 -  Removed the '#nullable disable' from the Libplanet.Data project. [[#3644]]
 -  Removed the '#nullable disable' from the Libplanet.Data.RocksDB project.
    [[#3651]]
...
### Added APIs
 ...
 -  (Libplanet.Data.Remote) Introduce
    `Libplanet.Data.Server.RemoteKeyValueService`  [[#3688]]
 -  (Libplanet.Data.Remote) Introduce
    `Libplanet.Data.Client.RemoteKeyValueStore`  [[#3688]]

### Behavioral changes

 -  (Libplanet.Data) Optimized `ITrie.IterateNodes()` to greatly
...
Released on February 22, 2024.
 ...
 -  (Libplanet.State) Fixed a bug where `FeeCollector.Mortgage()`
...
Released on February 20, 2024.
 ...
 -  (Libplanet.State) Optimized `ActionEvaluation` by removing
...
Released on January 26, 2024.
 ...
 -  (Libplanet.State) Changed `World.SetAccount()` to throw an
...
### Backward-incompatible API changes
 ...
 -  Bumped `BlockHeader.CurrentProtocolVersion` to 5.  [[#3524]]
 -  Removed `BlockChain.GetBalance(Address, Currency, Address)` method.
    [[#3583]]
 -  Removed `BlockChain.GetTotalSupply(Currency, Address)` method.
    [[#3583]]
 -  (Libplanet.State) Changed `BlockExecutor` to accept `World`
    instead of `Account`.  [[#3462]]
 -  (Libplanet.State) `ActionEvaluation.OutputState` became `World`.
    (was `Account`)  [[#3462]]
 -  (Libplanet.State) `IAction.Execute()` became to return `World`.
    (was `Account`)  [[#3462]]
 -  (Libplanet.State) `IActionContext.PreviousState` became `World`.
    (was `Account`)  [[#3462]]
 -  (Libplanet.State) Following methods in `IFeeCollector` interface
    became to accept and return `World`. (was `Account`)  [[#3462]]
     -  `IFeeCollector.Mortgage()`
     -  `IFeeCollector.Refund()`
     -  `IFeeCollector.Reward()`
 -  (Libplanet.State) `IBlockChainStates` interface has been overhauled.
    [[#3462], [#3583]]
 ...
 -  (@planetarium/tx)  Remove the `T` generic argument of `SignedTx<T>`.
    [[#3512]]
 -  (Libplanet.Types) Removed `EnumerableExtensions` class.  [[#3625], [#3626]]

### Added APIs

 -  Added `BlockHeader.LegacyStateVersion` constant.  [[#3524]]
 -  (Libplanet.State) Added `World` interface and its implementation.
    [[#3462]]
     -  Added `World` class.
 -  (Libplanet.State) Added `IWorldDelta` interface.  [[#3462]]
 -  (Libplanet.State) Added `IWorldState` interface and its implementation.
    [[#3462]]
     -  Added `WorldBaseState` class.
 -  (Libplanet.State) Added `ReservedAddresses` static class.  [[#3462]]
 -  (Libplanet.Data) Added `TrieMetadata` class.  [[#3540]]
