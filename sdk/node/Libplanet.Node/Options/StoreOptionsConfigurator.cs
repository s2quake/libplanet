    ...
    public void Configure(StoreOptions options)
    {
        if (options.Type == StoreType.Memory)
        {
            if (options.RootPath != string.Empty)
            {
                ...
                logger.LogWarning("RootPath is ignored because StoreType is Memory.");
            }

            if (options.StoreName != string.Empty)
            {
                options.StoreName = string.Empty;
                logger.LogWarning("StorePath is ignored because StoreType is Memory.");
            }

            if (options.StateStoreName != string.Empty)
            {
                options.StateStoreName = string.Empty;
                logger.LogWarning("StateStorePath is ignored because StoreType is Memory.");
            }
        }
        else
        {
