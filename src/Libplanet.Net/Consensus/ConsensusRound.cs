...
internal sealed class ConsensusRound(int round, Consensus consensus)
{
    ...
    public VoteBox PreVotes { get; } = new(consensus.Height, round, VoteType.PreVote, consensus.Validators);

    public VoteBox PreCommits { get; } = new(consensus.Height, round, VoteType.PreCommit, consensus.Validators);

    public MajorityBox PreVoteMajorities { get; } = new(consensus.Height, round, VoteType.PreVote, consensus.Validators);

    public MajorityBox PreCommitMajorities { get; } = new(consensus.Height, round, VoteType.PreCommit, consensus.Validators);
    ...
    //     {
    //         var message = $"Maj23 round {maj23.Round} does not match expected round {Round}";
    //         throw new ArgumentException(message, nameof(maj23));
    //     }
