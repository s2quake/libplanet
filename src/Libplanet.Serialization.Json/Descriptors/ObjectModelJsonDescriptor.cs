...
internal sealed class ObjectModelJsonDescriptor : ModelJsonDescriptor
{
    public override bool CanSerialize(Type type)
        => type.IsDefined(typeof(ModelAttribute)) || type.IsDefined(typeof(OriginModelAttribute));

    public override (string, Type)[] GetTypes(Type type, out bool isArray)
    {
        isArray = false;
        var properties = ModelResolver.GetProperties(type);
        var types = new (string, Type)[properties.Length];
        for (var i = 0; i < properties.Length; i++)
        {
            types[i] = (properties[i].Name, properties[i].PropertyType);
        }
