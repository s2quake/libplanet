...
public class DefaultStore : StoreBase
{
    ...
    // private readonly SubFileSystem _blockHashes;
    ...
    public DefaultStore(DefaultStoreOptions options)
    {
        ...
        if (options.Path == string.Empty)
        {
            ...
            _blocks = new BlockCollection(new DefaultKeyValueStore());
            _txExecutions = new TxExecutionCollection(new DefaultKeyValueStore());
            _blockHashes = new BlockHashCollection(new DefaultKeyValueStore());
            _blockCommits = new BlockCommitCollection(new DefaultKeyValueStore());
            _nextStateRootHashes = new StateRootHashCollection(new DefaultKeyValueStore());
            _pendingEvidence = new EvidenceCollection(new DefaultKeyValueStore());
            _committedEvidence = new EvidenceCollection(new DefaultKeyValueStore());
        ...
        else
        {
            ...
            _blocks = new BlockCollection(new DefaultKeyValueStore(Path.Combine(options.Path, "block")));
            _txExecutions = new TxExecutionCollection(new DefaultKeyValueStore(Path.Combine(options.Path, "txexec")));
            _blockHashes = new BlockHashCollection(new DefaultKeyValueStore(Path.Combine(options.Path, "txbindex")));
            _blockCommits = new BlockCommitCollection(new DefaultKeyValueStore(Path.Combine(options.Path, "blockcommit")));
            _nextStateRootHashes =
                new StateRootHashCollection(new DefaultKeyValueStore(Path.Combine(options.Path, "nextstateroothash")));
            _pendingEvidence = new EvidenceCollection(new DefaultKeyValueStore(Path.Combine(options.Path, "evidencep")));
            _committedEvidence = new EvidenceCollection(new DefaultKeyValueStore(Path.Combine(options.Path, "evidencec")));
        ...
        // _blockHashes = new SubFileSystem(_root, TxIdBlockHashRootPath, owned: false);
    ...
    public override void PutTxIdBlockHashIndex(TxId txId, BlockHash blockHash)
    {
        ...
        // CreateDirectoryRecursively(_blockHashes, dirPath);
        // _blockHashes.WriteAllBytes(path, blockHash.Bytes.ToArray());
    }
    ...
    public override IEnumerable<BlockHash> IterateTxIdBlockHashIndex(TxId txId)
    {
        ...
        // if (!_blockHashes.DirectoryExists(txPath))
        // {
        //     yield break;
        // }

        // foreach (var path in _blockHashes.EnumerateFiles(txPath))
        // {
    ...
    public override void DeleteTxIdBlockHashIndex(TxId txId, BlockHash blockHash)
    {
        ...
        // var path = TxIdBlockHashIndexPath(txId, blockHash);
        // if (_blockHashes.FileExists(path))
        // {
        //     _blockHashes.DeleteFile(path);
        // }
    }
