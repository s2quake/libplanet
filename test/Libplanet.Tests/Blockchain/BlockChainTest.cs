...
public partial class BlockChainTest : IDisposable
{
    ...
    public void MakeTransactionWithSystemAction()
    {
        ...
        Assert.Equal(ModelSerializer.Serialize(action), transaction.Actions[0]);
    ...
    private void ValidateNextBlockCommitOnValidatorSetChange()
    {
        ...
        Assert.Equal(
            blockChain
                ...
                .GetValidatorSet(),
            [.. ValidatorPrivateKeys.Select(pk => new Validator(pk.PublicKey, BigInteger.One))]);

        Assert.Equal(
            blockChain
                .GetNextWorldState(1L)
                .GetValidatorSet(),
            [.. newValidators.Select(pk => new Validator(pk.PublicKey, BigInteger.One))]);
    }
