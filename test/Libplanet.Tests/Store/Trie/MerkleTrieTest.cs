...
public class MerkleTrieTest
{
    ...
    public void ToDictionary()
    {
        ...
        var trie = Libplanet.Store.Trie.Trie.Create([
            (KeyBytes.Create([0xbe, 0xef]), Dictionary.Empty),
            (KeyBytes.Create([0x01]), Null.Value),
            (KeyBytes.Create([0x02]), Null.Value),
            (KeyBytes.Create([0x03]), Null.Value),
            (KeyBytes.Create([0x04]), Null.Value)]);
    ...
    public void IterateNodes()
    {
        ...
        var trie = Libplanet.Store.Trie.Trie.Create(
            (KeyBytes.Create(0xbe, 0xef), Dictionary.Empty.Add(GetRandomBytes(32), Null.Value)));
